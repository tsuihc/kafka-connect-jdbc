...
public abstract class BaseDialectTest<T extends GenericDatabaseDialect> {
  ...
  protected void assertMapping(
    ...
    Assertions.assertEquals(expectedSqlType, sqlType);
  }
  ...
  protected void assertColumnConverter(
    ...
    Assertions.assertNotNull(dialect.columnConverterFor(
      mapping,
      mapping.columnDefn(),
  ...
  protected Map<String, String> propertiesFromPairs(String... pairs) {
    ...
    Assertions.assertEquals(0,
                            pairs.length % 2,
                            "Expecting even number of properties but found " + pairs.length);
  ...
  protected Collection<ColumnId> columns(TableId id,
                                         String... names) {
  ...
  protected void verifyDataTypeMapping(String expected,
                                       Schema schema) {
    SinkRecordField field = new SinkRecordField(schema, schema.name(), schema.isOptional());
    Assertions.assertEquals(expected, dialect.getSqlType(field));
  }

  protected void verifyCreateOneColNoPk(String expected) {
    Assertions.assertEquals(expected, dialect.buildCreateTableStatement(tableId, Arrays.asList(
  ...
  protected void verifyAlterAddOneCol(String... expected) {
    Assertions.assertArrayEquals(expected, dialect.buildAlterTable(tableId, Arrays.asList(
  ...
  @Test
  public void bindFieldPrimitiveValuesExceptByteAndStringAndBytes()
  throws SQLException {
  ...
  @Test
  public void bindFieldByteValue()
  throws SQLException {
    int index = ThreadLocalRandom.current().nextInt();
  ...
  @Test
  public void bindFieldStringValue()
  throws SQLException {
    int index = ThreadLocalRandom.current().nextInt();
  ...
  @Test
  public void bindFieldBytesValue()
  throws SQLException {
    int index = ThreadLocalRandom.current().nextInt();
  ...
  protected void assertSanitizedUrl(String url,
                                    String expectedSanitizedUrl) {
    Assertions.assertEquals(expectedSanitizedUrl, dialect.sanitizedUrl(url));
  }

  protected PreparedStatement verifyBindField(int index,
                                              Schema schema,
                                              Object value)
  throws SQLException {
