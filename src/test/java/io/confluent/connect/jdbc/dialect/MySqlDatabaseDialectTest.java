...
public class MySqlDatabaseDialectTest extends BaseDialectTest<MySqlDatabaseDialect> {
  ...
  public void select() {
    TableId actor = tableId("actor");
    String expected = "select `actor`.`id`,`actor`.`pad` from `actor` where `id` = ?";
    String sql = dialect.buildSelectStatement(actor, columns(actor, "id", "pad"), columns(actor, "id"));
    Assertions.assertEquals(expected, sql);

    expected = "select * from `actor` where `id` = ?";
    sql = dialect.buildSelectStatement(actor, new ArrayList<>(), columns(actor, "id"));
    Assertions.assertEquals(expected, sql);
  }
