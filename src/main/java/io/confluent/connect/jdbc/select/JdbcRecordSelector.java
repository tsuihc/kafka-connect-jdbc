...
public class JdbcRecordSelector {
  ...
  private Map<ColumnId, ColumnDefinition> columnDefs;
  ...
  throws SQLException, TableAlterOrCreateException {
    ...
    if (schemaChanged || selectStatement == null) {
      ...
      columnDefs = dialect.describeColumnsByQuerying(connection, tableId);
    }
    bindKeyFields(keys);
  ...
  protected void bindField(int index,
                           ...
                           String fieldName)
  throws SQLException {
    ColumnDefinition columnDef = columnDefs.get(new ColumnId(tableId, fieldName));
  ...
  private List<ColumnId> getKeyColumns(Schema keySchema) {
    ...
    if (keySchema.type().isPrimitive()) {
      keyColumns.add(keySchema.schema().name());
    } else {
      for (Field field : keySchema.schema().fields()) {
        keyColumns.add(field.name());
      }
    }
    return keyColumns.stream().map(keyColumn -> new ColumnId(tableId, keyColumn)).collect(Collectors.toList());
  }
